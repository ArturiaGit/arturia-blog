...
glossary:
  RAG:
    title: RAG
    brief: 检索增强生成（Retrieval-Augmented Generation），先从外部知识中检索相关片段，再基于这些片段生成答案。
  Chunking:
    title: Chunking（文档切分）
    brief: 把长文档按语义或结构切成较小片段，便于检索与上下文拼接。
  Embedding:
    title: Embedding（向量化）
    brief: 将文本映射为高维向量表示，使系统可以按语义相似度进行检索。
  向量检索:
    title: 向量检索
    brief: 在向量空间中查找与查询最相近的内容片段，常用于语义搜索。
---
